#!/usr/bin/env python3
"""Add bare-wh-question annotations to existing CAS XMI files.

Detects bare wh-questions: elliptical sentences consisting of just a
wh-phrase ("Why?", "What for?", "About what?", "What man?",
"How viable?") — no verb, no embedding governor. The missing predicate
is recoverable from the preceding discourse.

Detection logic lives in :mod:`preprocessing.detection.bare_questions`
...
Annotation added per finding:
  - GrammarAnomaly: description="Ellipsis", category="bare_wh"

(No "QEmbedder" LexicalPhrase — bare wh-questions have no governor.)

Usage:
    # Single file, default view (_InitialView):
    python add_bare_questions.py input.xmi

    # Directory of XMI files, specific views:
    python add_bare_questions.py ./xmi_dir/ --view _InitialView spelling_corrected

    # Custom output directory (default: overwrite in place):
    python add_bare_questions.py ./xmi_dir/ --output ./annotated/
"""
...
def process_file(
    ...
    *,
    ...
    """Load an XMI file, detect bare wh-questions on the views, and save."""
    ...
    for view_name in views:
        ...
        existing = _existing_bare_question_annotations(view)
        if existing:
            if not replace:
                logger.info(
                    f"{xmi_path.name}: view '{view_name}' already has "
                    f"{len(existing)} bare-wh-question annotations, skipping "
            ...
            logger.info(
                ...
                f"{len(existing)} existing bare-wh-question annotations."
            )

        count = find_and_annotate_bare_questions(
            view, ts, lang=lang, mixed=mixed
        )
        logger.info(
            f"{xmi_path.name}: view '{view_name}' — {count} bare wh-questions found"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Add bare-wh-question annotations to existing CAS XMI files."
    )
    parser.add_argument(
        "input",
        type=Path,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Remove existing bare-wh-question annotations on each "
        "processed view before re-running the detector. Default: skip "
        "views that already have bare-wh-question annotations.",
    )
