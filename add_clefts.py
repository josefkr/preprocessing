#!/usr/bin/env python3
"""Add cleft-construction annotations to existing CAS XMI files.

Currently detects English **it-clefts**:

  - The focus head F is a nominal (UPOS in {NOUN, PRON, PROPN}) with
    deprel ``root`` or ``ccomp``.
  - F has a child with ``deprel == cop`` whose lemma is ``be``.
  - F has a child with ``deprel == acl:relcl``.
  - F has a child with ``deprel ∈ {nsubj, expl}`` whose lemma is ``it``.

Detection logic lives in :mod:`preprocessing.detection.clefts` and
...
Annotations added per finding:
  - LexicalPhrase, text="Cleft_focus", over the focused NP.
  - LexicalPhrase, text="Cleft_presupposition", over the relative-
    clause subtree.
  - LexicalPhrase, text="Cleft_it", on the cleft pronoun.

Usage:
    python add_clefts.py input.xmi
    python add_clefts.py ./xmi_dir/ --view _InitialView spelling_normalized
    python add_clefts.py ./xmi_dir/ --output ./annotated/
"""
...
def process_file(
    ...
    *,
    ...
    """Load an XMI file, detect clefts on the views, and save."""
    ...
    for view_name in views:
        ...
        existing = _existing_cleft_annotations(view)
        if existing:
            if not replace:
                logger.info(
                    f"{xmi_path.name}: view '{view_name}' already has "
                    f"{len(existing)} cleft annotations, skipping "
            ...
            logger.info(
                ...
                f"{len(existing)} existing cleft annotations."
            )

        count = find_and_annotate_clefts(view, ts, lang=lang, mixed=mixed)
        logger.info(
            f"{xmi_path.name}: view '{view_name}' — {count} cleft cases found"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Add cleft-construction annotations to existing CAS XMI files."
    )
    parser.add_argument(
        "input",
        type=Path,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Remove existing cleft annotations on each processed "
        "view before re-running the detector. Default: skip views "
        "that already have cleft annotations.",
    )
