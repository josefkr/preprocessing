#!/usr/bin/env python3
"""Add gapped-coordination annotations to existing CAS XMI files.

Detects coordinated clauses where the second (or later) conjunct
lacks its main predicate — the verb (or, in copular sentences, the
copula) has to be supplied from the antecedent conjunct.

Example: "Paul wanted a milk shake and Mr Leonard a coffee."
                                       └─ gapped clause; "wanted"
                                          must be borrowed.

Detection logic lives in
:mod:`preprocessing.detection.gapped_coordination` and operates on
udapi trees, so it can be tested independently with ``.conllu``
fixtures. This script is a thin CAS/CLI wrapper.

Annotations added per finding:
  - GrammarAnomaly: description="Ellipsis", category="gapped_coordination"
    over the gapped clause span.
  - LexicalPhrase: text="GappedAntecedent" on the matrix verb whose
    predicate the gap is borrowing.

Usage:
    # Single file, default view (_InitialView):
    python add_gapped_coordination.py input.xmi

    # Directory of XMI files, specific views:
    python add_gapped_coordination.py ./xmi_dir/ --view _InitialView spelling_normalized

    # Custom output directory (default: overwrite in place):
    python add_gapped_coordination.py ./xmi_dir/ --output ./annotated/
"""
...
def _existing_gapped_coordination_annotations(view) -> list:
    """All annotations this detector would have created on ``view``."""
    out = [
        a for a in view.select(T_GRAMMAR_ANOMALY)
        if getattr(a, "category", None) == "gapped_coordination"
    ]
    out.extend(
        lp for lp in view.select(T_LEXICAL_PHRASE)
        if getattr(lp, "text", None) == "GappedAntecedent"
    )
    return out
...
def process_file(
    ...
    *,
    ...
    """Load an XMI file, detect gapped coordination on the views, and save."""
    ...
    for view_name in views:
        ...
        existing = _existing_gapped_coordination_annotations(view)
        if existing:
            if not replace:
                logger.info(
                    f"{xmi_path.name}: view '{view_name}' already has "
                    f"{len(existing)} gapped-coordination annotations, skipping "
            ...
            logger.info(
                ...
                f"{len(existing)} existing gapped-coordination annotations."
            )

        count = find_and_annotate_gapped_coordination(
            view, ts, lang=lang, mixed=mixed
        )
        logger.info(
            f"{xmi_path.name}: view '{view_name}' — {count} gapped-coordination cases found"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Add gapped-coordination annotations to existing CAS XMI files."
    )
    parser.add_argument(
        "input",
        type=Path,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Remove existing gapped-coordination annotations on each "
        "processed view before re-running the detector. Default: skip "
        "views that already have gapped-coordination annotations.",
    )
