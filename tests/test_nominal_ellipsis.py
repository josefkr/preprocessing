...
def test_quantifier():
    f = _only("positive_quantifier_en.conllu")
...
def test_none():
    f = _only("positive_none_en.conllu")
    assert f.subtype == "none"
    assert f.text.lower() == "none"


def test_numeral():
    f = _only("positive_numeral_en.conllu")
...
def test_every_one():
    f = _only("positive_every_one_en.conllu")
...
def test_comparative():
    f = _only("positive_comparative_en.conllu")
...
def test_elder():
    ...
    f = _only("positive_elder_en.conllu")
    assert f.subtype == "elder"
    assert f.text.lower() == "elder"


def test_adjective():
    f = _only("positive_adjective_en.conllu")
...
def test_amod_modifier_is_not_ellipsis():
    findings = detect_nominal_ellipsis(_load("negative_amod_en.conllu"))
    assert findings == []


def test_restrict_to_lang_filters_other_languages():
    findings = detect_nominal_ellipsis(
        _load("positive_quantifier_en.conllu"), restrict_to_lang="de"
    )
    assert findings == []
